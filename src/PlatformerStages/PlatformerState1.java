...
public class PlatformerState1 extends GameState {
	...
	public void init() {
		...
		tileMap = new TileMap(gsm.getPlatformerMap(gsm.getPlatformerCourse() - 1),
				gsm.getTileset(gsm.getPlatformerCourse() - 1));
	...
	private void populateEnemies() {
		...
		for (int[] o : enemyObjects) {
			switch (o[0]) {
			case 40:
				TriangleHead s = new TriangleHead(tileMap);
				s.setPosition(o[1], o[2]);
				enemies.add(s);
				break;
			case 28:
				Pickup p = new Pickup(tileMap);
				p.setPosition(o[1] + tileMap.getTileSize() / 2, o[2] + tileMap.getTileSize() / 2);
				enemies.add(p);
				break;

			case 29:
				Door d = new Door(tileMap, 10);
			...
			case 30:
				player = new Player(tileMap, character, attributes);
				startX = o[1] + tileMap.getTileSize() / 2;
				startY = o[2] + tileMap.getTileSize() / 2;
				player.setPosition(startX, startY);
				break;
			case 31:
				CreepyGuy c = new CreepyGuy(tileMap);
	...
	public void update() {
		...
		for (int i = 0; i < enemies.size(); i++) {
			...
			if (e.gety() > tileMap.getHeight() - (2 * tileMap.getTileSize())) {
				e.hit(0.1);
			}
			if (e.isDead()) {
				enemies.remove(i);
		...
		// if player reaches bottom of screen, decrement score and reposition
		// player from beginning
	...
	public void draw(Graphics2D g) {
		...
		g.drawString("YEAR: " + String.valueOf(((gsm.getPlatformerCourse() - 1) / gsm.getNumCourses() + 1)),
				hudsection * 3 + 15, 12);
	...
	public void keyPressed(int k) {
		if (k == KeyEvent.VK_LEFT && !lockPlayer)
			player.setLeft(true);
		if (k == KeyEvent.VK_RIGHT && !lockPlayer)
			player.setRight(true);
		if (k == KeyEvent.VK_UP && !lockPlayer)
			player.setUp(true);
		if (k == KeyEvent.VK_DOWN && !lockPlayer)
			player.setDown(true);
		if (k == KeyEvent.VK_W && !lockPlayer) {
